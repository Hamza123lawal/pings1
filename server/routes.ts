...
export async function registerRoutes(
  ...
  app.post("/api/orders", async (req, res) => {
    try {
      ...
      // Clear cart and respond immediately — do NOT await emails
      ...
      if (gmailTransport) {
        ...
        gmailTransport.sendMail({
          ...
          `,
        }).catch((err: any) => console.error("[GMAIL] Admin email failed:", err));
        ...
        gmailTransport.sendMail({
          ...
          `,
        }).then(() => console.log(`[GMAIL] Customer confirmation sent to ${order.customerEmail}`))
          .catch((err: any) => console.error("[GMAIL] Customer email failed:", err));
      }
    } catch (err) {
      if (err instanceof z.ZodError) {
